...
{
public:
	SGD(PredictiveModel *mdl, double(*loss_funct)(const vector<double> &got, const vector<float> &y));
	void Learn(const vector<vector<float>> &xData, const vector<float> &yData, int T_Steps);
	//use one of these techniques to set params for subgradients
	double(*subGradientI)(int param_number, const vector<double> &param_values, const vector<vector<float>> &x, const vector<float> &y);
	...
	void set_special_step_func(double(*function)(const vector<double> &, const vector<float> &, const vector<double> &));
...
private:
	...
	double(*step_loss_function)(const vector<double> &got, const vector<float> &y, const vector<double> &model_params);
