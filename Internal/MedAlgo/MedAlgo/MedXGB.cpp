...
int MedXGB::Predict(float *x, float *&preds, int nsamples, int nftrs) const
{
	...
	if (XGDMatrixCreateFromMat(x, nsamples, nftrs, params.missing_value, &h_test) != 0)
		MTHROW_AND_ERR("failed to XGDMatrixCreateFromMat: %s\n", XGBGetLastError());

	bst_ulong out_len;
	const float *out_preds;
	if (XGBoosterPredict(my_learner, h_test, 0, 0, 0, &out_len, &out_preds) != 0)
		MTHROW_AND_ERR("failed to XGBoosterPredict: %s\n", XGBGetLastError());
...
{
	DMatrixHandle matrix_handles[2] = {nullptr, nullptr};
	int n_matrices = 1;
...
#pragma omp critical
	...
	for (int iter = 1; iter < params.num_round; iter++)
	{
		...
		{
			if (params.validate_frac > 0)
			{
				XGBoosterEvalOneIter(h_booster, iter, matrix_handles, evnames, 2, &out_result);
			}
			else
			{
				XGBoosterEvalOneIter(h_booster, iter, matrix_handles, evnames, 1, &out_result);
			}
	...
	{
		if (matrix_handles[i] != nullptr)
			XGDMatrixFree(matrix_handles[i]);
	}
	return 0;
}
...
bool split_token(const string &str, const string &search, bool first, string &result)
{
...
int MedXGB::set_params(map<string, string> &mapper)
{
	...
	for (auto entry : mapper)
	{
		...
		//! [MedXGB::init]
		if (field == "booster")
			params.booster = entry.second;
		else if (field == "objective")
			params.objective = entry.second;
		else if (field == "eval_metric")
			split(params.eval_metric, entry.second, boost::is_any_of(","));
		else if (field == "eta")
			params.eta = stof(entry.second);
		else if (field == "gamma")
			params.gamma = stof(entry.second);
		else if (field == "min_child_weight")
			params.min_child_weight = stoi(entry.second);
		else if (field == "max_depth")
			params.max_depth = stoi(entry.second);
		else if (field == "num_round")
			params.num_round = stoi(entry.second);
		else if (field == "silent")
			params.silent = stoi(entry.second);
		else if (field == "num_class")
			params.num_class = stoi(entry.second);
		else if (field == "missing_value")
			params.missing_value = stof(entry.second);
		else if (field == "colsample_bytree")
			params.colsample_bytree = stof(entry.second);
		else if (field == "colsample_bylevel")
			params.colsample_bylevel = stof(entry.second);
		else if (field == "subsample")
			params.subsample = stof(entry.second);
		else if (field == "scale_pos_weight")
			params.scale_pos_weight = stof(entry.second);
		else if (field == "tree_method")
			params.tree_method = entry.second;
		else if (field == "lambda")
			params.lambda = stof(entry.second);
		else if (field == "alpha")
			params.alpha = stof(entry.second);
		else if (field == "seed")
			params.seed = stoi(entry.second);
		else if (field == "verbose_eval")
			params.verbose_eval = stoi(entry.second);
		else if (field == "split_penalties")
			params.split_penalties = entry.second;
		else if (field == "monotone_constraints")
			params.monotone_constraints = entry.second;
		else if (field == "validate_frac")
			params.validate_frac = stof(entry.second);

		else
			MLOG("Unknonw parameter \'%s\' for XGB\n", field.c_str());
		//! [MedXGB::init]
	}
...
void MedXGB::prepare_predict_single()
{
	...
	{
		BoosterHandle temp_handler = nullptr;
		if (XGBoosterCreate(nullptr, 0, &temp_handler) != 0)
			throw runtime_error(string("failed XGBoosterCreate\n") + XGBGetLastError());
		if (XGBoosterLoadModelFromBuffer(temp_handler, out_dptr, len) != 0)
			throw runtime_error(string("failed XGBoosterLoadModelFromBuffer\n") + XGBGetLastError());
...
{
	int n_ftrs = static_cast<int>(x.size());
	DMatrixHandle h_test = NULL;
	if (XGDMatrixCreateFromMat(x.data(), 1, n_ftrs, params.missing_value, &h_test) != 0)
		MTHROW_AND_ERR("failed to XGDMatrixCreateFromMat: %s\n", XGBGetLastError());
	...
	BoosterHandle booster = learner_per_thread[n_th];

	// for each thread learner
	// xgb_mdl->Predict(h_test, false, &wrapper, 0, 0, false, false, false, false);
	bst_ulong out_len;
	const float *out_preds;
	if (XGBoosterPredict(booster, h_test, 0, 0, 0, &out_len, &out_preds) != 0)
		MTHROW_AND_ERR("XGBoosterPredict failed: %s\n", XGBGetLastError());

	preds.assign(out_preds, out_preds + out_len);
