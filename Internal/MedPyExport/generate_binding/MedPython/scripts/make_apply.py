...
parser = re.compile(r'MEDPY_NP_(?P<iotype>INPUT|OUTPUT|INPLACE|VARIANT_OUTPUT)\(\s*(?P<params>.*?)\s*\)')
...
"""
def main(args):
  from os.path import isfile,isdir,splitext, join as joinpath, abspath, exists as pathexists
  from os import walk
  if len(args)<=2:
    print(args[0] + ' - A tool to generate function definitions for numpy.i using a macro in the code')
    print('usage: ' + args[0] + ' [input_file_name|input_dir_name] [output_file_name]')
    return
  in_fname , out_fname =  args[1:3]
  if isfile(in_fname):
    print('parsing ' + in_fname)
    file_parse(in_file=open(in_fname), 
...
def file_parse(in_file, out_file):
  ...
  for line in in_file:
    if 'class' in line:
      ...
      if m:
        ...
        if cur_class not in renamed_class and cur_class.startswith('MP'):
          print('%rename({}) {};'.format(cur_class[2:],cur_class),file=out_file)
    ...
    if ('MEDPY_GET_' in line or 'MEDPY_SET_' in line) and not line.startswith('#define'):
  ...
  for cur_class in prop_list:
    print('%extend {}{{\n  %pythoncode %{{'.format(cur_class), file=out_file)
    prop_def = {}
