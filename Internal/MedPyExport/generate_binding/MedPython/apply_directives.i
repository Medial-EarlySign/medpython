...
%apply (int** ARGOUTVIEWM_ARRAY1, int* DIM1) {(int** intkeys_out_buf, int* intkeys_out_buf_len), (int** int_out_buf, int* int_out_buf_len), (int** intkeys_out_buf, int* intkeys_out_buf_len)}
%apply (float** ARGOUTVIEWM_ARRAY1, int* DIM1) {(float** float_out_buf, int* float_out_buf_len)}
%apply (float* IN_ARRAY1, int DIM1) {(float* float_in_buf, int float_in_buf_len)}
%apply (int* IN_ARRAY1, int DIM1) {(int* int_in_buf, int int_in_buf_len)}
%rename(Example6) MPExample6;
%apply (double* INPLACE_ARRAY1, int DIM1) {(double* vec, int m)}
%apply (double** ARGOUTVIEWM_ARRAY1, int* DIM1) {(double** array, int* n), (double** array11, int* n11), (double** array22, int* n22)}
%apply (void** ARGOUTMVAR_ARRAY1, int* DIM1, int* NPYDTC1) {(void** var_arr, int* var_arr_sz, int* var_arr_type)}
%apply (double* IN_ARRAY1, int DIM1) {(double* vec2, int nn)}
%extend MedPyExportExample4{
  %pythoncode %{
    __swig_getmethods__["x"] = MEDPY_GET_x
    __swig_setmethods__["x"] = MEDPY_SET_x
    __swig_getmethods__["ro_x"] = MEDPY_GET_ro_x
    __swig_setmethods__["wo_x"] = MEDPY_SET_wo_x
    if _newclass:
      ro_x = property(MEDPY_GET_ro_x,None)
      x = property(MEDPY_GET_x,MEDPY_SET_x)
      wo_x = property(None,MEDPY_SET_wo_x)
  %}
};
