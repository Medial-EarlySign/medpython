...
MPMat::MPMat(float *IN_ARRAY2, int DIM1, int DIM2) { o = new MedMat<float>(IN_ARRAY2, DIM1, DIM2); };
...
void MPMat::get_numpy_copy(float** ARGOUTVIEWM_ARRAY2, int* DIM1, int* DIM2) {
	auto sz = o->size();
...
void MPMat::get_numpy_view_unsafe(float** ARGOUTVIEW_ARRAY2, int* DIM1, int* DIM2) {
	*ARGOUTVIEW_ARRAY2 = o->m.data();
	*DIM1 = o->nrows;
	*DIM2 = o->ncols;
}

void MPMat::load_numpy(float* IN_ARRAY2, int DIM1, int DIM2) {
	o->load(IN_ARRAY2, DIM1, DIM2);
}


void MPMat::test(float** ARGOUTVIEWM_ARRAY2, int* DIM1, int* DIM2,int n_rows, int n_cols) {
...
void MPMat::MEDPY_GET_row_ids(MEDPY_NP_OUTPUT(int** out_row_ids_buf, int* out_row_ids_buf_len)) {
	vector_to_buf(o->row_ids, out_row_ids_buf, out_row_ids_buf_len);
}
void MPMat::MEDPY_SET_row_ids(MEDPY_NP_INPUT(int* row_ids_buf, int row_ids_buf_len)) {
	buf_to_vector(row_ids_buf, row_ids_buf_len, o->row_ids);
}

void MPMat::MEDPY_GET_avg(MEDPY_NP_OUTPUT(int** avg_buf, int* avg_buf_len)) {
	vector_to_buf(o->avg, avg_buf, avg_buf_len);
}

void MPMat::MEDPY_GET_std(MEDPY_NP_OUTPUT(int** std_buf, int* std_buf_len)) {
...
void MPMat::add_rows(MEDPY_NP_INPUT(float* m_add, int nrows_to_add)) { o->add_rows(m_add, nrows_to_add); };
void MPMat::add_cols(MPMat& m_add) { o->add_cols(*(m_add.o)); };
void MPMat::add_cols(MEDPY_NP_INPUT(float* m_add, int ncols_to_add)) { o->add_cols(m_add, ncols_to_add); };

void MPMat::get_row(int i_row, MEDPY_NP_OUTPUT(float** rowv, int* rowv_n)) const {
	vector<float> ret;
	o->get_row(i_row, ret);
	vector_to_buf(ret, rowv, rowv_n);
}
void MPMat::get_col(int i_col, MEDPY_NP_OUTPUT(float** colv, int* colv_n)) const {
	vector<float> ret;
	o->get_col(i_col, ret);
...
void MPMat::normalize(MEDPY_NP_INPUT(float* external_avg, int external_avg_n),
	MEDPY_NP_INPUT(float* external_std, int external_std_n), int norm_type) {
...
void MPMat::get_cols_avg_std(MEDPY_NP_OUTPUT(float** buf_avg, int* buf_avg_n), MEDPY_NP_OUTPUT(float** buf_std, int* buf_std_n)) {
...
void MPMat::load(float *IN_ARRAY2, int DIM1, int DIM2) { o->load(IN_ARRAY2, DIM1, DIM2); };
void MPMat::load_transposed(float *IN_ARRAY2, int DIM1, int DIM2) { o->load_transposed(IN_ARRAY2, DIM1, DIM2); };
