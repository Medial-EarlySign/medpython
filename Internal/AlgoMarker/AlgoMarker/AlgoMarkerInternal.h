...
public:
	...
	float fetch_threshold(const string &threshold, string &err_msg) const {
		...
		if (tokens.size() != 2) {
			err_msg = "Error flag_threshold should contain $";
			return MED_MAT_MISSING_VALUE;
		}
		mes_trim(tokens[0]);
		mes_trim(tokens[1]);
		if (mbr.find(tokens[0]) == mbr.end()) {
			err_msg = "Error flag_threshold doesn't contain threshold settings for " + tokens[0];
			return MED_MAT_MISSING_VALUE;
		}
		...
		if (meas_tokens.size() != 2) {
			err_msg = "Error flag_threshold doesn't should contain _ in the cutoff setting part";
			return MED_MAT_MISSING_VALUE;
		}
		float num_val;
		try {
			...
			err_msg = "Error flag_threshold search cutoff isn't numeric";
			return MED_MAT_MISSING_VALUE;
		}
		...
		if (res == MED_MAT_MISSING_VALUE)
			err_msg = "Error flag_threshold doesn't contain threshold for " + tokens[1];
		return res;
	}
};
