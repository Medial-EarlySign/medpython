...
so_functions* so_functions::so = nullptr;
std::vector<so_functions> so_functions::sos;
...
void load_am(const char * am_fname){
  if(so_functions::load(am_fname)<0)
    exit(0);
}

int so_functions::load(const char * am_fname){
...
int AM_API_ClearData(AlgoMarker * pAlgoMarker){
  return (*((so_functions::t_AM_API_ClearData)so_functions::so->AM_API_ClearData))
    (pAlgoMarker);
}

void AM_API_DisposeAlgoMarker(AlgoMarker * pAlgoMarker){
  (*((so_functions::t_AM_API_DisposeAlgoMarker)so_functions::so->AM_API_DisposeAlgoMarker))
    (pAlgoMarker);
}

void AM_API_DisposeRequest(AMRequest *pRequest){
  (*((so_functions::t_AM_API_DisposeRequest)so_functions::so->AM_API_DisposeRequest))
    (pRequest);
}

void AM_API_DisposeResponses(AMResponses *responses){
  (*((so_functions::t_AM_API_DisposeResponses)so_functions::so->AM_API_DisposeResponses))
    (responses);
}

int AM_API_GetResponseScoresNum(AMResponse *response, int *n_scores){
  return (*((so_functions::t_AM_API_GetResponseScoresNum)so_functions::so->AM_API_GetResponseScoresNum))
    (response, n_scores);
}
int AM_API_GetName(AlgoMarker * pAlgoMArker, char **name){
  return (*((so_functions::t_AM_API_GetName)so_functions::so->AM_API_GetName))
    (pAlgoMArker, name);
}
int AM_API_GetScoreMessages(AMResponse *response, int score_index, int *n_msgs, int **msgs_codes, char ***msgs_args){
  return (*((so_functions::t_AM_API_GetScoreMessages)so_functions::so->AM_API_GetScoreMessages))
...
int AM_API_GetResponseMessages(AMResponse *response, int *n_msgs, int **msgs_codes, char ***msgs_args){
  return (*((so_functions::t_AM_API_GetResponseMessages)so_functions::so->AM_API_GetResponseMessages))
...
int AM_API_GetResponseScoreByType(AMResponses *responses,int res_index, char *_score_type, float *out_score){
  return (*((so_functions::t_AM_API_GetResponseScoreByType)so_functions::so->AM_API_GetResponseScoreByType))
...
int AM_API_GetResponseScoreByIndex(AMResponse *response, int score_index, float *score, char **_score_type){
  return (*((so_functions::t_AM_API_GetResponseScoreByIndex)so_functions::so->AM_API_GetResponseScoreByIndex))
    (response,score_index,score,_score_type);
}

int AM_API_GetResponsePoint(AMResponse *response, int *pid, long long *timestamp){
  return (*((so_functions::t_AM_API_GetResponsePoint)so_functions::so->AM_API_GetResponsePoint))
    (response,pid,timestamp);
}

int AM_API_GetSharedMessages(AMResponses *responses, int *n_msgs, int **msgs_codes, char ***msgs_args){
  return (*((so_functions::t_AM_API_GetSharedMessages)so_functions::so->AM_API_GetSharedMessages))
    (responses,n_msgs,msgs_codes,msgs_args);
}

int AM_API_GetResponsesNum(AMResponses *responses){
   return (*((so_functions::t_AM_API_GetResponsesNum)so_functions::so->AM_API_GetResponsesNum))
     (responses);
}
int AM_API_GetResponseIndex(AMResponses *responses, int _pid, long long _timestamp){
   return (*((so_functions::t_AM_API_GetResponseIndex)so_functions::so->AM_API_GetResponseIndex))
     (responses,_pid,_timestamp);
}
int AM_API_GetResponseIndex(AMResponses *responses, char **requestId){
   return (*((so_functions::t_AM_API_GetResponsesRequestId)so_functions::so->AM_API_GetResponsesRequestId))
     (responses, requestId);
}

int AM_API_GetResponseAtIndex(AMResponses *responses, int index, AMResponse **response){
   return (*((so_functions::t_AM_API_GetResponseAtIndex)so_functions::so->AM_API_GetResponseAtIndex))
     (responses,index,response);
}

int AM_API_Create(int am_type, AlgoMarker **new_am){
  return (*((so_functions::t_AM_API_Create)so_functions::so->AM_API_Create))
    (am_type, new_am);
}

int AM_API_Load(AlgoMarker * pAlgoMarker, const char *config_fname){
  return (*((so_functions::t_AM_API_Load)so_functions::so->AM_API_Load))
...
int AM_API_AddData(AlgoMarker * pAlgoMarker, int patient_id, const char *signalName, int TimeStamps_len, long long* TimeStamps, int Values_len, float* Values){
  return (*((so_functions::t_AM_API_AddData)so_functions::so->AM_API_AddData))
...
int AM_API_AddDataStr(AlgoMarker * pAlgoMarker, int patient_id, const char *signalName, int TimeStamps_len, long long* TimeStamps, int Values_len, char** Values) {
	return (*((so_functions::t_AM_API_AddDataStr)so_functions::so->AM_API_AddDataStr))
		(pAlgoMarker, patient_id, signalName, TimeStamps_len, TimeStamps, Values_len, Values);
}
...
int AM_API_CreateRequest(char *requestId, char **score_types, int n_score_types, int *patient_ids, long long *time_stamps, int n_points, AMRequest **new_req){
  return (*((so_functions::t_AM_API_CreateRequest)so_functions::so->AM_API_CreateRequest))
...
int AM_API_CreateResponses(AMResponses **new_responses){
  return (*((so_functions::t_AM_API_CreateResponses)so_functions::so->AM_API_CreateResponses))
    (new_responses);
}

int AM_API_Calculate(AlgoMarker *pAlgoMarker, AMRequest *request, AMResponses *responses){
  return (*((so_functions::t_AM_API_Calculate)so_functions::so->AM_API_Calculate))
