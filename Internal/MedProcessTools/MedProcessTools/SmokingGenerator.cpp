...
int SmokingGenerator::Generate(PidDynamicRec& rec, MedFeatures& features, int index, int num) {
	...
	for (int i = 0; i < num; i++) {
		...
		float __pack_years;
		int len;
		__current_smoker = __ex_smoker = __never_smoker = __unknown_smoker = __years_since_quitting = __smoking_years = __smoking_quantity = __plm_smoking_level = __pack_years = MED_MAT_MISSING_VALUE;
		int qa_print = 0;
		...
		if (smoking_method != "SMOKING_ENRICHED") {
						...
						smoke_desc[4] = "current not ex unknown";
								...
								if (test_date >= start_date) {
									...
									if (smoke_ranges[kk].val == 7) {
										...
										if (diff > 0) {
											...
											int temp_pack_years = 0;
											if (ex_smoking_quan != missing_val) temp_pack_years += diff*ex_smoking_quan;
											else temp_pack_years += diff*SMOKING_QUANTITY_IMPUTE;
											pack_years += temp_pack_years;
							...
							float unknown_f = 0;
							float quantity_f = 0;
							for (int kk = smoke_ranges.size() - 1; kk >= 0; kk--) {
								int start_date = smoke_ranges[kk].date_start;
								int end_date = smoke_ranges[kk].date_end;

								if (test_date >= start_date && test_date <= end_date) {
									int val = smoke_ranges[kk].val;
							...
							__current_smoker = smoke_f;
							__ex_smoker = ex_smoke_f;
							__never_smoker = never_smoke_f;
							__unknown_smoker = unknown_f;
							__years_since_quitting = years_since_q_f;
							__smoking_years = smoking_year_ff;
							__smoking_quantity = quantity_f;
							__pack_years = pack_years_ff;

							if (qa_print == 1) {
								fprintf(stderr, "====== final : \n");
								fprintf(stderr, "years_since_q_f %f \n", __years_since_quitting);
								fprintf(stderr, "never_smoke_f %f \n", __never_smoker);
								fprintf(stderr, "ex_smoke_f %f \n", __ex_smoker);
								fprintf(stderr, "smoke_f %f \n", __current_smoker);
								fprintf(stderr, "unknown_f %f \n", __unknown_smoker);
								fprintf(stderr, "quantity_f  %f \n", __smoking_quantity);
								fprintf(stderr, "smoking_year_f: %f \n", __smoking_years);
		...
		if (__current_smoker == missing)
			...
			float packs_per_day = __pack_years / __smoking_years;
		...
		for (int j = 0; j < names.size(); j++) {
			...
			if (algorithm::ends_with(names[j], "Current_Smoker"))
				p_feat[i] = (float)__current_smoker;
			else if (algorithm::ends_with(names[j], "Ex_Smoker"))
				p_feat[i] = (float)__ex_smoker;
			else if (algorithm::ends_with(names[j], "Smok_Years_Since_Quitting"))
				p_feat[i] = (float)__years_since_quitting;
			else if (algorithm::ends_with(names[j], "Smoking_Years"))
				p_feat[i] = (float)__smoking_years;
			else if (algorithm::ends_with(names[j], "Smok_Pack_Years")) 
				p_feat[i] = (float)__pack_years;
			else if (algorithm::ends_with(names[j], "PLM_Smoking_Level"))
				p_feat[i] = (float)plm_smoking_level;
			else if (algorithm::ends_with(names[j], "Never_Smoker"))
				p_feat[i] = (float)__never_smoker;
			else if (algorithm::ends_with(names[j], "Unknown_Smoker"))
				p_feat[i] = (float)__unknown_smoker;
			else if (algorithm::ends_with(names[j], "Smoking_Quantity"))
				p_feat[i] = (float)__smoking_quantity;
...
int SmokingGenerator::Generate(PidDynamicRec& rec, MedFeatures& features, int index, int num) {
	...
	for (int i = 0; i < num; i++) {
		...
		string sname = "SMOKING_ENRICHED";
		SValShort4 *smx_status = (SValShort4 *)rec.get(sname, i, len);
		if (len > 0)
			never_smoked = (smx_status[0].val1 == -1); // never smoked is -1, -1, 0, 0
		assert(len <= 1);
