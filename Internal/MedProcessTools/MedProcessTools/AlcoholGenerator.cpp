...
int AlcoholGenerator::Generate(PidDynamicRec& rec, MedFeatures& features, int index, int num) {
	...
	for (int i = 0; i < num; i++) {
		...
		int qa_print = 0;
			...
			string sname = "Alcohol_quantity";
			SDateShort2 *drk_info = (SDateShort2 *)rec.get(sname, i, len);
		...
		for (int j = 0; j < names.size(); j++) {
			...
			if (algorithm::ends_with(names[j], "Current_Drinker"))
				p_feat[i] = (float)__current_drinker;
			else if (algorithm::ends_with(names[j], "Ex_Drinker"))
				p_feat[i] = (float)__ex_drinker;
			else if (algorithm::ends_with(names[j], "Drinker_Years_Since_Quitting"))
				p_feat[i] = (float)__years_since_quitting;
			else if (algorithm::ends_with(names[j], "Drinker_Years"))
				p_feat[i] = (float)__drinker_years;
			else if (algorithm::ends_with(names[j], "Drink_Unit_Years"))
				p_feat[i] = (float)__unit_years;
			else if (algorithm::ends_with(names[j], "PLM_Drinking_Level"))
				p_feat[i] = (float)plm_drinking_level;
			else if (algorithm::ends_with(names[j], "Never_Drinker"))
				p_feat[i] = (float)__never_drinker;
			else if (algorithm::ends_with(names[j], "Unknown_Drinker"))
				p_feat[i] = (float)__unknown_drinker;
			else if (algorithm::ends_with(names[j], "Drinker_Quantity"))
				p_feat[i] = (float)__drinker_quantity;
			else if (algorithm::ends_with(names[j], "Current_Alcoholic"))
				p_feat[i] = (float)__current_alcoholist;
			else if (algorithm::ends_with(names[j], "Ex_Alcoholic"))
				p_feat[i] = (float)__ex_alcoholist;
