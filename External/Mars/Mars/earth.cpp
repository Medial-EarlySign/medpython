...
static void *malloc1(size_t size, const char *args, ...)
{
    ...
    if (!p || TraceGlobal == 1.5) {
        if (args == NULL)
            ...
            vsprintf(s, args, p);
            va_end(p);
            printf("malloc %s: %s\n", sFormatMemSize((int)size, true), s);
        }
    }
    if (!p)
        error("Out of memory (could not allocate %s)", sFormatMemSize((int)size, false));
    return p;
}
...
double my_pred_f(double *x, int n)
{
	double split[5] = {20,40,60,80,50};
	double lta[5] = {-3, -1, 0, 1, 3};
	double gta[5] = {0.3, -8.1, 2.1, -2.1, 4.5};

	double y = 0;
	for (int j=0; j<5; j++) {
		//y += sin((double)(j+1)*x[j]);
		if (x[2*j] < split[j])
			y += lta[j]*x[2*j];
		else
			y += gta[j]*x[2*j];
...
int run_mars_example()
{
    const int    nMaxTerms = 30;
    const int    nCases = 500000;
    const int    nResp = 1;     // number of y columns
    const int    nPreds = 50;
    const int    nMaxDegree = 3;
	...
	double *x1 = (double *)malloc1(nPreds*sizeof(double),NULL);
    ...
    for (int i = 0; i < nCases; i++) {
		...
		for (int j=0; j<nPreds; j++) {
			x1[j] = (double)100*(double)rand()/(double)RAND_MAX;
			//x[i*nPreds+j] = (double)100*(double)rand()/(double)RAND_MAX;
		}
		y[i] = my_pred_f(x1,nPreds);
		for (int j=0; j<nPreds; j++) {
			x[j*nCases+i] = x1[j];
		}
	...
	for (int i=0; i<10; i++) {
		...
		PredictEarth(&y1, x1, BestSet, Dirs, Cuts, Betas, nPreds, nResp, nTerms, nMaxTerms);
		printf("\nf(%g) = %g %g\n", x1[0], y1, my_pred_f(x1,nPreds));
	}
    free1(LinPreds);
    free1(BestSet);
    free1(bx);
