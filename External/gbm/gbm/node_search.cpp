...
)
{
	...
	if(cCurrentVarClasses == 0)   // variable is continuous
    {
        if(dLastXValue > dX)
        {
            fprintf(stderr,
				"Observations are not in order. gbm() was unable to build an index for the design matrix. Could be a bug in gbm or an unusual data type in data (%f > %f).\n",
				dLastXValue,dX);
            hr = GBM_FAIL;
            goto Error;
        }
...
GBMRESULT CNodeSearch::EvaluateCategoricalSplit()
{
    ...
    my_rsort_with_index(adGroupMean,aiCurrentCategory,cCurrentVarClasses);
