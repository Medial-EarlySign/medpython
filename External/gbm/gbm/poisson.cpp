...
)
{
    ...
    if(adOffset == NULL)
    {
        for(iObs=0; iObs<nTrain; iObs++)
        {
            ...
            vecdMax[aiNodeAssign[iObs]] = 
               my_fmax2(adF[iObs],vecdMax[aiNodeAssign[iObs]]);
            vecdMin[aiNodeAssign[iObs]] =  
               my_fmin2(adF[iObs],vecdMin[aiNodeAssign[iObs]]);
        }
    }
    else
    {
    ...
    for(iNode=0; iNode<cTermNodes; iNode++)
    {
        if(vecpTermNodes[iNode]!=NULL)
        {
            ...
            vecpTermNodes[iNode]->dPrediction = 
               my_fmin2(vecpTermNodes[iNode]->dPrediction,
                     19-vecdMax[iNode]);
            vecpTermNodes[iNode]->dPrediction = 
               my_fmax2(vecpTermNodes[iNode]->dPrediction,
                     -19-vecdMin[iNode]);
        }
    }
