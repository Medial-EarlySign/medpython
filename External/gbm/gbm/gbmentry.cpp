// Code used for calling from R, not used in Medial's code. 
// An unsatifsified ifdef is used to ignore it.
//
// GBM by Greg Ridgeway  Copyright (C) 2003


#ifdef __NeverDefined__
...
)
{
    ...
    {
       fprintf(stderr,"Iter   TrainDeviance   ValidDeviance   StepSize   Improve\n");
    }
    for(iT=0; iT<cTrees; iT++)
    {
            ...
            (iT==cTrees-1))
        {
            ...
            if(INTEGER(rfVerbose)[0])
            {
               fprintf(stderr,"%6d %13.4f %15.4f %10.4f %9.4f\n",
    ...
    if(INTEGER(rfVerbose)[0]) fprintf(stderr,"\n");
...
)
{
   ...
   {
      while(iTree < INTEGER(racTrees)[iPredictionIter])
      {
         rThisTree     = VECTOR_ELT(rTrees,iTree);
         aiSplitVar    = INTEGER(VECTOR_ELT(rThisTree,0));
         adSplitCode   = REAL   (VECTOR_ELT(rThisTree,1));
         ...
         for(iObs=0; iObs<cRows; iObs++)
         {
            ...
            adPredF[iObs] += dPred; // add the prediction
         } // iObs
