...
#pragma omp parallel for schedule(static)
	...
	std::vector<std::pair<size_t, std::string>> GBDT::FeatureImportance(const std::string &method) const {
		if (valid_feature_importance_methods.find(method) == valid_feature_importance_methods.end())
		{
			fprintf(stderr, "Error in LightGBM::GBDT.FeatureImportance - unknown method %s",
				method.c_str());
			throw std::invalid_argument("LightGBM::GBDT.FeatureImportance invalid method param");
		}
		...
		for (size_t i = 0; i < feature_importances.size(); ++i) {
			if (feature_importances[i] > 0) {
				pairs.emplace_back((size_t)feature_importances[i], feature_names_[i]);
			}
		}
		// sort the importance
